...
export default async function DashboardPage(props: {
  ...
  // 🟢 【關鍵修正】：動態解析所有的屬性名稱（如 age, birthday, education 等）
  // 排除掉不該獨立成欄位的 'customer_info' 與可能誤入的大/小寫 'metadata' 字眼
    ...
    <div className="p-8 bg-gray-50 min-h-screen text-black text-[10px]">
      <div className="max-w-[98%] mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold uppercase tracking-tighter">Excel-Style CRM (Public)</h1>
        ...
        {isCustomerFiltering && (
          <div className="mb-10 animate-in fade-in duration-500">
            ...
            <div className="flex justify-center items-center gap-2 mt-4">
                <a href={`/dashboard?filteredPage=${currentFilteredPage - 1}`} className="px-2 py-1 border rounded bg-white hover:bg-gray-100 transition">Prev</a>
                <JumpToPage totalPages={filteredTotalPages} currentPage={currentFilteredPage} paramName="filteredPage" />
                <a href={`/dashboard?filteredPage=${currentFilteredPage + 1}`} className="px-2 py-1 border rounded bg-white hover:bg-gray-100 transition">Next</a>
            </div>
          </div>
        )}
        ...
        {/* 🟢 MAIN TABLE (實現全 Excel 直線封閉、完全移除獨立大 METADATA 欄位) */}
          ...
          <table className="min-w-full border-collapse border border-gray-300">
            <thead>
              <tr className="bg-gray-800 text-white uppercase tracking-wider text-[10px] divide-x divide-gray-700">
                ...
                {/* 1. 將包含 Excel 匯入及 Add Column 產生的所有客製屬性標頭展開 */}
                {allDynamicKeys.map((key) => (
                ...
                {/* 2. 【成功後移】：Add Column 放在客製化展平屬性的最右邊 */}
        ...
        <div className="flex justify-center items-center gap-2 mt-6 mb-10">
          <a href={`/dashboard?page=1`} className={`px-3 py-1 border-2 border-slate-800 rounded font-bold text-xs shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:bg-slate-100 active:translate-y-0.5 ${currentPage === 1 ? 'bg-slate-200 pointer-events-none' : 'bg-white'}`}>FIRST</a>
          <a href={`/dashboard?page=${currentPage - 1}`} className={`px-3 py-1 border-2 border-slate-800 rounded font-bold text-xs shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:bg-slate-100 ${currentPage <= 1 ? 'opacity-30 pointer-events-none' : 'bg-white'}`}>PREV</a>
          <div className="flex gap-1 px-4 py-1 bg-slate-800 text-white rounded font-mono text-xs">{currentPage} / {totalPages}</div>
          <a href={`/dashboard?page=${currentPage + 1}`} className={`px-3 py-1 border-2 border-slate-800 rounded font-bold text-xs shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:bg-slate-100 ${currentPage >= totalPages ? 'opacity-30 pointer-events-none' : 'bg-white'}`}>NEXT</a>
          <a href={`/dashboard?page=${totalPages}`} className={`px-3 py-1 border-2 border-slate-800 rounded font-bold text-xs shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:bg-slate-100 ${currentPage === totalPages ? 'bg-slate-200 pointer-events-none' : 'bg-white'}`}>LAST</a>
        ...
        {isSyncFiltering && (
          <div className="mt-6 animate-in slide-in-from-top-2 duration-300 max-w-md">
                    ...
                    {filteredSyncRecords.map((s: any) => {
                      ...
                      return (
                        <tr key={`filtered-sync-${s.id}`} className="bg-amber-50/20 divide-x divide-amber-100">
                          ...
                          <td className="px-4 py-2 italic text-gray-700 border border-amber-100">{safeEmailStr}</td>
                        </tr>
                      );
                    })}
                  </tbody>
            ...
            <div className="flex justify-center items-center gap-2 mt-4">
                <a href={`/dashboard?filteredSyncPage=${currentFilteredSyncPage - 1}`} className="px-2 py-1 border rounded bg-white hover:bg-gray-100 transition">Prev</a>
                <JumpToPage totalPages={filteredSyncTotalPages} currentPage={currentFilteredSyncPage} paramName="filteredSyncPage" />
                <a href={`/dashboard?filteredSyncPage=${currentFilteredSyncPage + 1}`} className="px-2 py-1 border rounded bg-white hover:bg-gray-100 transition">Next</a>
            </div>
          </div>
        )}
